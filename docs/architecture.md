...
1. **Local playback**: it initializes `ovos_audio.service.PlaybackService` and
...
WebSocket bus. `ovos-audio` and PHAL are wired to this FakeBus at startup.
`hivemind-core` forwards all OCP messages onto the FakeBus, where the local
audio stack consumes them.
